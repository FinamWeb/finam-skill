...
Заявка не может быть отменена так как она уже исполнена
...
func main() {
    ...
    req, _ := http.NewRequest("DELETE", "https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders/YOUR_ORDER_ID", nil)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .delete("https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders/YOUR_ORDER_ID")
        .bearer_auth(token)
        .send().await?;
...
curl -X DELETE 'https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders/YOUR_ORDER_ID' \
