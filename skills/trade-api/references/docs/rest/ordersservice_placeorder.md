...
| --- | --- | --- | --- |
| symbol | string | no |  |
| quantity | object | no | A representation of a decimal value, such as 2.5. Clients may convert values
...
| type | enum | no | - ORDER_TYPE_UNSPECIFIED: Значение не указано
 ...
 - ORDER_TYPE_MULTI_LEG: Мульти лег заявка — ORDER_TYPE_UNSPECIFIED, ORDER_TYPE_MARKET, ORDER_TYPE_LIMIT, ORDER_TYPE_STOP, ORDER_TYPE_STOP_LIMIT, ORDER_TYPE_MULTI_LEG |
| time_in_force | enum | no | - TIME_IN_FORCE_UNSPECIFIED: Значение не указано
 ...
 - TIME_IN_FORCE_FOK: Исполнить полностью или отменить — TIME_IN_FORCE_UNSPECIFIED, TIME_IN_FORCE_DAY, TIME_IN_FORCE_GOOD_TILL_CANCEL, TIME_IN_FORCE_GOOD_TILL_CROSSING, TIME_IN_FORCE_EXT, TIME_IN_FORCE_ON_OPEN, TIME_IN_FORCE_ON_CLOSE, TIME_IN_FORCE_IOC, TIME_IN_FORCE_FOK |
...
Services **should** error with `400 Bad Request` (`INVALID_ARGUMENT` in
gRPC) if the service receives a value outside of the supported range. |
| stop_price | object | no | A representation of a decimal value, such as 2.5. Clients may convert values
into language-native decimal formats, such as Java's [BigDecimal][] or
Python's [decimal.Decimal][].
...
Services **should** error with `400 Bad Request` (`INVALID_ARGUMENT` in
gRPC) if the service receives a value outside of the supported range. |
| stop_condition | enum | no | - STOP_CONDITION_UNSPECIFIED: Значение не указано
 - STOP_CONDITION_LAST_UP: Цена срабатывания больше текущей цены
 - STOP_CONDITION_LAST_DOWN: Цена срабатывания меньше текущей цены — STOP_CONDITION_UNSPECIFIED, STOP_CONDITION_LAST_UP, STOP_CONDITION_LAST_DOWN |
| legs | ordersLeg[] | no |  |
| legs.symbol | string | no |  |
| legs.quantity | object | no | A representation of a decimal value, such as 2.5. Clients may convert values
into language-native decimal formats, such as Java's [BigDecimal][] or
Python's [decimal.Decimal][].
...
Services **should** error with `400 Bad Request` (`INVALID_ARGUMENT` in
gRPC) if the service receives a value outside of the supported range. |
| legs.side | enum | no | - SIDE_UNSPECIFIED: Сторона сделки не указана
 - SIDE_BUY: Покупка
 - SIDE_SELL: Продажа — SIDE_UNSPECIFIED, SIDE_BUY, SIDE_SELL |
...
const response = await fetch('https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders', {
  method: 'POST',
  headers: {
...
response = requests.post(
    "https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders",
    headers={
...
func main() {
  ...
  "comment": "string"
    ...
    req, _ := http.NewRequest("POST", "https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders", bytes.NewBuffer(body))
...
$ch = curl_init('https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
curl_setopt($ch, CURLOPT_HTTPHEADER, ['Authorization: Bearer ' . $token, 'Content-Type: application/json']);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['symbol' => 'string', 'quantity' => ['value' => 'string'], 'side' => 'SIDE_UNSPECIFIED', 'type' => 'ORDER_TYPE_UNSPECIFIED', 'time_in_force' => 'TIME_IN_FORCE_UNSPECIFIED', 'limit_price' => ['value' => 'string'], 'stop_price' => ['value' => 'string'], 'stop_condition' => 'STOP_CONDITION_UNSPECIFIED', 'legs' => ['0' => ['symbol' => 'string', 'quantity' => ['value' => 'string'], 'side' => 'SIDE_UNSPECIFIED']], 'client_order_id' => 'string', 'valid_before' => 'VALID_BEFORE_UNSPECIFIED', 'comment' => 'string']));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .post("https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders")
        .bearer_auth(token)
        .json(&json!({
          "symbol": "string",
          "quantity": {
...
curl -X POST 'https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/orders' \
